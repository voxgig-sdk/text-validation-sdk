...
[MCP](https://modelcontextprotocol.io) server exposing the TextValidation SDK as
two agent tools — `text-validation_list` and `text-validation_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/text-validation-mcp)
make build

# 2. Provide credentials via the environment
export TEXT_VALIDATION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user text-validation \
  -- /absolute/path/to/text-validation-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./text-validation-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// text-validation_list: first page of records
{ "entity": "validation" }
{ "entity": "validation", "query": { } }

// text-validation_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/text-validation-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user text-validation \
     -- "$PWD"/dist/*/text-validation-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `text-validation_list` and `text-validation_load` tools now appear
   in new sessions. Ask the agent to *"list validation using text-validation"*
   and it calls `text-validation_list` with `{"entity":"validation"}`.
...
```sh
export TEXT_VALIDATION_APIKEY=sk_live_xxx            # API key
export TEXT_VALIDATION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `text-validation_list` tool
...
### Call the `text-validation_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `text-validation_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `text-validation_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TEXT_VALIDATION_APIKEY` | API key sent with every request. |
| `TEXT_VALIDATION_BASE` | Optional override of the API base URL. |
...
```sh
./text-validation-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"text-validation_load","arguments":{"entity":"validation","query":{"id":1}}}}'
```
