...
boru-driven command-line client **and** interactive REPL for the TextValidation
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/text-validation-cli)
make build

# 2. See usage (words, entities, env vars)
./text-validation-cli --help

# 3. Provide credentials once, via the environment
export TEXT_VALIDATION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./text-validation-cli load 1 validation            # {id:1} shorthand
./text-validation-cli load '{id:1}' validation       # explicit match map

# 5. Override the API base URL for a single call
TEXT_VALIDATION_BASE=https://api.example.com ./text-validation-cli load 1 validation

# 6. No arguments -> interactive REPL
./text-validation-cli
text-validation> load 1 validation
text-validation> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/text-validation-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/text-validation-cli load 1 validation
   ```
...
```sh
./text-validation-cli load 1 validation          # scalar shorthand for {id:1}
./text-validation-cli load '{id:1}' validation     # explicit match map
```
...
./text-validation-cli load 1 validation
```
...
Run with no arguments to open a REPL (prompt `text-validation>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/text-validation-cli`. |
...
(`./text-validation-cli <expr>`) and interactively (the REPL), and expressions compose the
