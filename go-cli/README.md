...
AQL-driven CLI and REPL for the TextValidation SDK. Positional arguments are
...
```sh
go build -o text-validation-cli ./...
```
...
./text-validation-cli list validation
./text-validation-cli load 1 validation
./text-validation-cli load '{id:1}' validation
