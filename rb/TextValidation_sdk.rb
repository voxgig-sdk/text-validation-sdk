...
require_relative 'TextValidation_types'
...
class TextValidationSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = TextValidationHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = TextValidationHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, TextValidationFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    TextValidationUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = TextValidationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = TextValidationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = TextValidationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => TextValidationError.new(
        "#{op}_allow",
        "TextValidationSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue TextValidationError => err
    ...
    if fetched.is_a?(Hash)
      status = TextValidationHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = TextValidationError.new(
        "graphql_error", "TextValidationSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = TextValidationSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
