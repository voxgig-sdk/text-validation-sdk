...
module TextValidationUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = TextValidationContext.new({}, nil)
    end
    op = ctx.op || TextValidationOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || TextValidationResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(TextValidationError) ? err.msg : err.to_s
    ...
    sdk_err = TextValidationError.new("", msg, ctx)
    sdk_err.result = ctx.utility.clean.call(ctx, result)
    sdk_err.spec = ctx.utility.clean.call(ctx, spec)
    sdk_err.code = err.code if err.is_a?(TextValidationError)
