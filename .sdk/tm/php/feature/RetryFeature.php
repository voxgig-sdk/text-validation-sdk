...
// TextValidation SDK retry feature
...
class TextValidationRetryFeature extends TextValidationBaseFeature
{
    ...
    public function init(TextValidationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (TextValidationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(TextValidationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(TextValidationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
