...
module TextValidationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = TextValidationResponse.new({})
    ...
    if fetch_err
      ...
      response = TextValidationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TextValidationResponse.new(fetched)
    else
