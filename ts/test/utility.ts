...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.TEXTVALIDATION_TEST_LIVE ||
    'TRUE' === process.env.TEXTVALIDATION_TEST_OVERRIDE
  ) {
  ...
  m.TEXTVALIDATION_TEST_EXPLAIN = process.env.TEXTVALIDATION_TEST_EXPLAIN || m.TEXTVALIDATION_TEST_EXPLAIN
