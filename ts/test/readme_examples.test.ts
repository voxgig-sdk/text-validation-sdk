...
//     (\`new TextValidationSDK(...)\` / \`TextValidationSDK.test(...)\` become
//     \`TextValidationSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'TextValidationSDK'
...
// rewrite real client construction (\`new TextValidationSDK(...)\` and any
// \`TextValidationSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TextValidationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TextValidationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
