...
// TextValidation SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class TextValidationMakeError
{
    public static function call(?TextValidationContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new TextValidationContext([], null);
        }
        $op = $ctx->op ?? new TextValidationOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new TextValidationResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof TextValidationError) ? $err->msg : (string)$err;
        $msg = "TextValidationSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new TextValidationError('', $msg, $ctx);
