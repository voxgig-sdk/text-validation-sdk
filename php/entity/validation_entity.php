...
class ValidationEntity
{
    ...
    {
        if ($args) {
            $this->_data = TextValidationHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = TextValidationHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   SDK boundary; throws TextValidationError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = TextValidationHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
