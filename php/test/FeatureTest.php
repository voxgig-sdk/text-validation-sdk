...
// TextValidation SDK feature test
//
...
class FtCtrl extends TextValidationControl
{
...
class FtClient
{
    ...
    private TextValidationUtility $utility;

    public function __construct(TextValidationUtility $utility, array $options)
    {
    ...
    public function get_utility(): TextValidationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TextValidationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public TextValidationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TextValidationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TextValidationConfig::make_feature($name);
    ...
    private static function buildUrl(TextValidationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TextValidationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TextValidationResponse($fetched);
        }
    ...
    private function fail(TextValidationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TextValidationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
