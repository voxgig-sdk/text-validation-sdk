# TextValidation SDK test runner
...
class TextValidationTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TextValidationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TextValidationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TextValidationTestRunner.getenv("TEXT_VALIDATION_TEST_LIVE")
        override = TextValidationTestRunner.getenv("TEXT_VALIDATION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TextValidationTestRunner.getenv(key)
        ...
        explain = TextValidationTestRunner.getenv("TEXT_VALIDATION_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if TextValidationTestRunner._test_control is not None:
            return TextValidationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TextValidationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TextValidationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TextValidationTestRunner.load_test_control()
...
def load_env_local():
    TextValidationTestRunner.load_env_local()


def env_override(m):
    return TextValidationTestRunner.env_override(m)


def entity_data(v):
    return TextValidationTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TextValidationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TextValidationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TextValidationTestRunner.load_test_control()


def live_client_options():
    return TextValidationTestRunner.live_client_options()


def live_delay_ms():
    return TextValidationTestRunner.live_delay_ms()
